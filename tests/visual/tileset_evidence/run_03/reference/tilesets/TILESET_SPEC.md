...
- TileSet: isometric, tile_size=(32,16), TILE_LAYOUT_DIAMOND_DOWN. Godot map_to_local(i,j) returns (16(i-j)+16,8(i+j)+8). Test/sample layers normalize their position to (-16,-8), so their parent-space cell centers are (16(i-j),8(i+j)). Character feet use layer.position + map_to_local(cell), then subtract elevation. All variants explicitly rendered with fixed screen upper-left light; never rotate finished PNGs.
...
- FLOOR_01 is top surface only; add PLATFORM_SIDE on exposed boundaries for thickness. FLOOR_EDGE exposes one side; CORNER_OUT exposes two full adjacent sides; CORNER_IN terminates the two half-edge faces at a reentrant junction while retaining the walkable top. Place the proper edge facing the exposed boundary.
- STAIR_SINGLE is one 8px level transition with two coarse 4px treads; STAIR_MULTI / ELEVATION_TRANSITION use four 2px treads. Ascent direction names the high end. Stair center test feet are y=-4; platform top feet y=-8.
- BRIDGE has 12×16 local-world walk surface (28px projected width), WALKWAY_NARROW 8×16 (24px projected width), both occupy one logical cell. BRIDGE_BROKEN has a real 6-unit gap: non-walkable in both worlds.
- ARCH has two obstruction polygons for its piers and a passable center. SUPPORT is a below-ground supporting column, not a walk surface. CLIFF top is at 0 and its side extends down 24px.
- Inner cutouts in vertical wall/pillar/arch faces are decorative segmentation, not holes in their obstruction polygons. No holes are cut in logically continuous walkable bridge silhouettes.
- Test/sample uses Y-sort for ground-level front/behind. Standing on elevated platforms, stairs, or bridges uses the upper surface draw stratum explicitly; this is a reusable placement convention, not an implemented movement system.
